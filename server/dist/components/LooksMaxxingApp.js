...
exports.default = LooksMaxxingApp;
const react_1 = __importStar(require("react"));
const intake_1 = require("@/agents/intake");
const recommendation_1 = require("@/agents/recommendation");
const shopping_1 = require("@/agents/shopping");
// Instantiate agents
const intakeAgent = new intake_1.IntakeAgent();
const recommendationAgent = new recommendation_1.RecommendationAgent();
const shoppingAgent = new shopping_1.ShoppingAgent();
function LooksMaxxingApp() {
    const [step, setStep] = (0, react_1.useState)('intake');
    const [profile, setProfile] = (0, react_1.useState)(null);
    const [routine, setRoutine] = (0, react_1.useState)(null);
    ...
    if (step === 'intake') {
        return (<div className="max-w-2xl mx-auto p-6 bg-white rounded-xl shadow-lg">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {['acne', 'aging', 'dryness', 'oiliness', 'pigmentation', 'sensitivity', 'frizz', 'damage', 'scalp_itch'].map(concern => (<button key={concern} onClick={() => toggleConcern(concern)} className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${formData.concerns.includes(concern)
          ...
          <div className="flex items-center">
            <input type="checkbox" name="fragranceFree" checked={formData.fragranceFree} onChange={handleInputChange} className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"/>
          ...
          <button onClick={handleSubmitIntake} className="w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 mt-6">
            Generate My Routine
          </button>
        </div>
      </div>);
    ...
    if (step === 'results' && routine) {
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div>
              ...
              <div className="space-y-4">
                {routine.skincareAM.map((step, idx) => (<div key={idx} className="bg-slate-50 p-3 rounded-lg">
            ...
            <div>
              ...
              <div className="space-y-4">
                {routine.skincarePM.map((step, idx) => (<div key={idx} className="bg-slate-50 p-3 rounded-lg">
                    <div className="text-sm font-bold text-slate-700 uppercase tracking-wide">{step.stepName}</div>
                    <div className="font-medium text-slate-900 mt-1">{step.product?.name}</div>
                    <div className="text-xs text-slate-500">{step.product?.brand}</div>
                    <p className="text-sm text-slate-600 mt-2 italic">"{step.instruction}"</p>
            ...
            <div>
              ...
              <div className="space-y-4">
                {routine.haircare.map((step, idx) => (<div key={idx} className="bg-slate-50 p-3 rounded-lg">
                    <div className="text-sm font-bold text-slate-700 uppercase tracking-wide">{step.stepName}</div>
                    <div className="font-medium text-slate-900 mt-1">{step.product?.name}</div>
                    <div className="text-xs text-slate-500">{step.product?.brand}</div>
                    <p className="text-sm text-slate-600 mt-2 italic">"{step.instruction}"</p>
        ...
        <div className="flex justify-center">
            <button onClick={() => setStep('cart')} className="py-3 px-8 bg-indigo-600 text-white rounded-full font-bold shadow-lg hover:bg-indigo-700 transition-transform transform hover:-translate-y-1">
    ...
    if (step === 'cart' && cart) {
        ...
        <div className="space-y-4 divide-y divide-slate-100">
          {cart.items.map((item, idx) => (<div key={idx} className="flex justify-between items-center py-2">
              <div>
        ...
        <div className="mt-8 space-y-3">
          {cart.retailerLinks.map((link, idx) => (<a key={idx} href={link.cartUrl} target="_blank" rel="noopener noreferrer" className="block w-full text-center py-3 bg-black text-white rounded-lg hover:bg-slate-800 transition-colors">
