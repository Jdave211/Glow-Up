...
exports.MATCH_PRODUCTS_FUNCTION = void 0;
 ...
 */
function buildHairQuery(profile) {
    const parts = [];
    if (profile.hairType) {
 ...
 */
async function hybridSearch(queryEmbedding, keywords, category, maxPrice, limit = 10) {
    // Build the query
    let query = supabase_1.supabase.rpc('match_products', {
    ...
    const boostedResults = results.map((p) => {
        let boost = 0;
    ...
    return boostedResults.slice(0, limit).map((p) => ({
        id: p.id,
        name: p.name,
        brand: p.brand,
        price: p.price,
 ...
 */
async function fallbackSearch(keywords, category, maxPrice, limit = 10) {
    ...
    if (skinTypes.length > 0) {
        const { data: skinResults, error } = await supabase_1.supabase
            .from('products')
            .select('*')
    ...
    if (concerns.length > 0) {
        const { data: concernResults, error } = await supabase_1.supabase
            .from('products')
            .select('*')
    ...
    for (const term of searchTerms) {
        const { data: textResults, error } = await supabase_1.supabase
            .from('products')
            .select('*')
    ...
    const scoredResults = uniqueResults.map((p) => {
        let score = 0.6; // Base score
        const productSkinTypes = (p.target_skin_type || []).map((t) => t.toLowerCase());
        const productConcerns = (p.target_concerns || []).map((t) => t.toLowerCase());
        ...
        for (const c of concerns) {
            if (productConcerns.some((pc) => pc.includes(c.toLowerCase()))) {
                score += 0.08;
            }
        }
        // Boost for rating
    ...
    return scoredResults.slice(0, limit).map((p) => ({
        id: p.id,
        name: p.name,
        brand: p.brand,
        price: p.price,
 ...
 */
async function executeInferenceTool(toolName, args) {
    try {
        switch (toolName) {
            case 'search_products': {
                ...
                if (results.length < limit) {
                    let q = supabase_1.supabase.from('products').select('*');
                    if (category)
                        q = q.eq('category', category);
                    if (skinType)
                        q = q.contains('target_skin_type', [skinType]);
                    if (maxPrice)
            ...
            case 'get_product_details': {
                let product = null;
                if (args.product_id) {
                    const { data } = await supabase_1.supabase.from('products').select('*').eq('id', args.product_id).single();
                    product = data;
                }
                else if (args.product_name) {
                    const { data } = await supabase_1.supabase.from('products').select('*').ilike('name', `%${args.product_name}%`).limit(1).single();
                    product = data;
                }
                if (!product)
 ...
 */
async function resolveProductFromDB(productName, productBrand) {
    try {
        // 1. Exact ilike match on name
        let query = supabase_1.supabase.from('products').select('*').ilike('name', `%${productName}%`);
        if (productBrand)
            query = query.ilike('brand', `%${productBrand}%`);
        ...
        if (shortName !== productName) {
            const { data: d2 } = await supabase_1.supabase.from('products').select('*')
...
function getFocusTargets(profile) {
    const base = {
        acne: {
            key: 'acne',
...
function hasTargetCoverage(routine, target) {
    const text = [
...
async function findTargetedProductForFocus(target, profile, presearched) {
    ...
    const fallback = await fallbackSearch([profile.skinType, ...target.queryTerms].filter(Boolean), target.preferredCategory, budgetMax, 5);
    if (fallback.length > 0)
        return fallback[0];
    return null;
}
async function ensureFocusCoverage(profile, routine, presearched) {
    const focusTargets = getFocusTargets(profile);
    if (focusTargets.length === 0)
        return { routine, addedFocus: [] };
    const used = new Set([...routine.morning, ...routine.evening, ...routine.weekly]
        .map(s => s.product?.id)
        .filter(Boolean));
    const addedFocus = [];
    const nextRoutine = {
    ...
    for (const target of focusTargets) {
        if (hasTargetCoverage(nextRoutine, target))
            continue;
        ...
        if (bucket !== 'weekly' && arr.length >= 5)
            continue;
        if (bucket === 'weekly' && arr.length >= 3)
            continue;
        arr.push({
            step: arr.length + 1,
...
function normalizeRoutineOrdering(routine) {
    const normalizeBucket = (steps, routineType) => (steps || [])
        .map((step, index) => ({
        ...step,
...
function getTechniqueTargets(profile) {
    const concernText = (profile.looksmaxConcerns || []).join(' ').toLowerCase();
    const has = (...tokens) => tokens.some((token) => concernText.includes(token));
    const targets = [];
...
function ensureTechniqueCoverage(profile, routine) {
    const targets = getTechniqueTargets(profile);
    if (targets.length === 0)
        return { routine, addedMethods: [] };
    const nextRoutine = {
        morning: [...routine.morning],
        evening: [...routine.evening],
        weekly: [...routine.weekly],
    ...
    for (const target of targets) {
        ...
        if (hasExisting)
            continue;
        if (target.routineType !== 'weekly' && bucket.length >= 5)
            continue;
        if (target.routineType === 'weekly' && bucket.length >= 3)
            continue;
        bucket.push({
 ...
 */
async function generatePersonalizedRoutine(profile, products) {
...
Search for products that match my profile, then build the full routine JSON with core skincare + optional technique steps if relevant.`;
    ...
    const toolProductsById = new Map();
    const toolProductsByName = new Map();
    for (const p of products) {
        if (p.id)
            toolProductsById.set(p.id, p);
        if (p.name)
            toolProductsByName.set(p.name.toLowerCase(), p);
    }
    try {
        let messages = [
                ...
                const toolResults = await Promise.all(assistantMsg.tool_calls.map(async (tc) => {
                    ...
                    try {
                        ...
                        if (parsed.products && Array.isArray(parsed.products)) {
                            ...
                            if (parsed.name)
                                toolProductsByName.set(parsed.name.toLowerCase(), parsed);
                        }
                    }
                    catch { }
        ...
        const resolveStep = async (s) => {
            ...
            if (!product && s.product_name) {
                ...
                const toolMatch = toolProductsByName.get(lowerName)
                    || [...toolProductsByName.values()].find((p) => p.name.toLowerCase().includes(lowerName) || lowerName.includes(p.name.toLowerCase()));
                if (toolMatch) {
                    product = {
            ...
            if (!product && s.product_name) {
                const preMatch = products.find(p => p.name.toLowerCase().includes(s.product_name.toLowerCase()) ||
            ...
            if (!product && s.product_id) {
                try {
                    const { data } = await supabase_1.supabase.from('products').select('*').eq('id', s.product_id).single();
                    if (data) {
                        product = {
            ...
            if (!product) {
                ...
                if (categoryGuess) {
                    ...
                    if (catProducts && catProducts.length > 0) {
                        ...
                        const pick = catProducts.find((cp) => !usedIds.has(cp.id)) || catProducts[0];
                        product = {
        ...
        const allCandidates = [
            ...products,
            ...Array.from(toolProductsById.values()).map((p) => ({
                id: p.id,
                name: p.name,
                brand: p.brand,
                price: p.price,
 ...
 */
function generateFallbackRoutine(profile, products) {
 ...
 */
async function semanticSearchWithSeedProducts(keywords, maxPrice, limit = 12) {
    ...
    const matchingConcerns = keywords.filter(k => concernTerms.some(c => k.toLowerCase().includes(c)));
    // Step 1: Find seed products that match user's profile
    // Only search skincare categories, not hair products
    let seedQuery = supabase_1.supabase
        .from('products')
    ...
    const { data: similarProducts, error: matchError } = await supabase_1.supabase.rpc('match_products', {
    ...
    const filteredProducts = (similarProducts || [])
        ...
        if (p.category && !SKINCARE_CATEGORIES.some(cat => p.category.toLowerCase().includes(cat)))
            return false;
        return true;
    });
    console.log(`✅ Semantic search found ${filteredProducts.length} skincare products (filtered from ${similarProducts?.length || 0})`);
    return filteredProducts.slice(0, limit).map((p) => ({
        id: p.id,
        name: p.name,
        brand: p.brand,
        price: p.price,
 ...
 */
async function runInference(profile) {
    ...
    if (skinEmbedding) {
        ...
        skincareProducts = await hybridSearch(skinEmbedding, keywords, undefined, maxPrice, 12);
    }
    ...
    for (const cat of essentialCategories) {
        if (!categories.has(cat) && skincareProducts.length < 15) {
            ...
            const { data: catProducts } = await supabase_1.supabase
                .from('products')
                .select('*')
                .eq('category', cat)
                .lte('price', maxPrice)
 ...
 */
exports.MATCH_PRODUCTS_FUNCTION = `
