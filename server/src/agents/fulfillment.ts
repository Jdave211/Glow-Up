...
interface OrderItem {
  ...
  name: string;
  brand: string;
  url: string;        // buy_link (Ulta product page)
  quantity: number;
  price: number;
}
...
const pendingOrders = new Map<string, { browser: Browser; page: Page; order: OrderRequest; logs: string[] }>();

export class FulfillmentAgent {
  private static MARKUP_PERCENTAGE = 0.15; // 15% service fee
  private static FREE_SHIPPING_THRESHOLD = 35; // Ulta free shipping over $35

  // ─────────────────────────────────────────────────
  // SETUP: Launch visible browser for manual Ulta login
  // Run this ONCE to save the session.
  // ─────────────────────────────────────────────────
  static async setupSession(): Promise<{ success: boolean; message: string }> {
  ...
  // ─────────────────────────────────────────────────
  static async processOrder(order: OrderRequest): Promise<OrderResult> {
    const logs: string[] = [];
          ...
          if (!clicked) {
            ...
            const fallback = await page.evaluate(() => {
              ...
              const addBtn = buttons.find(b =>
                b.innerText.toLowerCase().includes('add to bag') && !b.disabled
              );
      ...
      if (!orderPlaced) {
        ...
        orderPlaced = await page.evaluate(() => {
          ...
          const btn = buttons.find(b =>
            b.innerText.toLowerCase().includes('place order') && !b.disabled
          );
